...
show_tab_wnd::show_tab_wnd(QString header, QString &sql, QSqlDatabase &db, s_data_app &data_app, QWidget *parent, bool f_hist) :
    header(header),
    sql(sql),
    db(db),
    data_app(data_app),
...
void show_tab_wnd::on_bn_toCSV_clicked() {
    ...
    QString fname_s = QFileDialog::getSaveFileName(this,
                                                   "Куда сохранить файл?",
                                                   data_app.path_out + QDate::currentDate().toString("yyyy-MM-dd") + "  -  " + data_app.smo_short + " - " + data_app.filial_name + "  -  " + this->windowTitle().replace(":","=").replace("*","@").replace("!","|") + ".csv",
