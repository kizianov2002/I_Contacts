...
void get_date_wnd::refresh_years_mons() {
    ...
    QString sql = "select min(req_dt) as date_min, max(req_dt) as date_max "
                  "  from public.contact_center ; ";
    mySQL.exec(this, sql, QString("Диапазон дат обращения застрахованных"), *query, true, db, data_app);
